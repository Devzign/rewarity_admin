...
export default function Profile() {
  ...
  useEffect(() => {
    // Refresh from storage in case opened directly
    setUser(getUser());
  ...
  const profileItems = useMemo(() => {
    return [
      ...
      {
        icon: "ri-key-2-line",
        text: token ? `${token.substring(0, 12)}...` : "Not stored",
        label: "JWT Token"
      }
    ];
  }, [user, token]);
          ...
          <Col xl>
            <div className="media-profile mb-5">
              ...
              <div className="media-body">
                <h5 className="media-name">{user?.userName || "Guest"}</h5>
                <p className="d-flex gap-2 mb-2">
                  <i className="ri-user-3-line"></i>
                  <span>{user?.email || "No email"}</span>
                </p>
                <p className="mb-0 text-muted">
                  <span className="me-3"><i className="ri-hashtag"></i> {user?.uniqueCode || "-"}</span>
                  <span><i className="ri-fingerprint-line"></i> {user?.id || user?._id || "-"}</span>
                </p>
              </div>
            </div>
